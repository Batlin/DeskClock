...
public final class StopwatchCircleView extends View {
    ...
    public StopwatchCircleView(Context context, AttributeSet attrs) {
        ...
        final float dotDiameter = resources.getDimension(com.androidinspain.deskclock.R.dimen.circletimer_dot_size);

        mDotRadius = dotDiameter / 2f;
        mScreenDensity = resources.getDisplayMetrics().density;
        mStrokeSize = resources.getDimension(com.androidinspain.deskclock.R.dimen.circletimer_circle_size);
        mMarkerStrokeSize = resources.getDimension(com.androidinspain.deskclock.R.dimen.circletimer_marker_size);
        mRadiusOffset = Utils.calculateRadiusOffset(mStrokeSize, dotDiameter, mMarkerStrokeSize);

        mRemainderColor = Color.WHITE;
        mCompletedColor = ThemeUtils.resolveColor(context, com.androidinspain.deskclock.R.attr.colorAccent);
