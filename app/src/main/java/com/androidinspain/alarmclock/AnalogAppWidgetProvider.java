...
package com.androidinspain.alarmclock;
...
public class AnalogAppWidgetProvider extends AppWidgetProvider {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        dm.updateWidgetCount(getClass(), widgetCount, com.androidinspain.deskclock.R.string.category_analog_widget);
    }
    ...
    public void onUpdate(Context context, AppWidgetManager wm, int[] widgetIds) {
        ...
        for (int widgetId : widgetIds) {
            ...
            final RemoteViews widget = new RemoteViews(packageName, com.androidinspain.deskclock.R.layout.analog_appwidget);

            // Tapping on the widget opens the app (if not on the lock screen).
            if (Utils.isWidgetClickable(wm, widgetId)) {
                final Intent openApp = new Intent(context, DeskClock.class);
                final PendingIntent pi = PendingIntent.getActivity(context, 0, openApp, 0);
                widget.setOnClickPendingIntent(com.androidinspain.deskclock.R.id.analog_appwidget, pi);
            }
